...
ExecStart=/usr/bin/trojan-go -config $CONFIG_FILE_YAML
Restart=on-failure
RestartSec=10s
...
EOF
    cat > $CONFIG_FILE_YAML <<-EOF
run-type: custom
# 要启用自定义协议栈，将run_type指定为custom，此时除inbound和outbound之外的其他选项将被忽略
# https://p4gefau1t.github.io/trojan-go/advance/customize-protocol-stack/
inbound:
  node:
    - protocol: websocket
      tag: websocket
